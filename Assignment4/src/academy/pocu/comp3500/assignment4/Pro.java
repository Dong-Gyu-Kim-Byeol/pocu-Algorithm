...
//    }
    ...
    private HashMap<Task, DirectedGraphNode<Task>> getGraph() {
        ...
        for (final Task task : this.tasks) {
            for (final Task preTask : task.getPredecessors()) {
                ...
                outPreNode.addNode(outGraph.get(task));
            }
        }
    ...
    private void addGhostNode(final Task ghostTask,
        ...
        for (final Task preTask : predecessors) {
            ...
            ghostNode.addNode(this.graph.get(preTask));
        }
...
//            ghostNode.removeNext(this.graph.get(preTask));
    ...
    private void maxFlow(final HashMap<Task, Boolean> skipOrNull,
        ...
        final LinkedList<IsTransposedFlow<Task>> path = new LinkedList<>();
        ...
        while (true) {
            ...
            for (final IsTransposedFlow<Task> isBackFlow : path) {
                final int index = indexes.get(isBackFlow.getData());
                if (isBackFlow.isTransposedFlow()) {
    ...
    private void bfsShortestPathSourceToSinkSkipZeroFlow(final HashMap<Task, Boolean> skipOrNull,
                                                         ...
                                                         final int[] backFlow,
                                                         final HashMap<Task, Integer> indexes,
                                                         final LinkedList<IsTransposedFlow<Task>> outPath,
        ...
        final LinkedList<IsTransposedFlow<Task>> bfsQueue = new LinkedList<>();

        final HashMap<IsTransposedFlow<Task>, IsTransposedFlow<Task>> prePath = new HashMap<>();
        ...
        IsTransposedFlow<Task> isBackFlow = null;
        while (!bfsQueue.isEmpty()) {
            ...
            for (final DirectedGraphNode<Task> nextNode : node.getNodes()) {
                ...
                final IsTransposedFlow<Task> nextIsBackFlow = new IsTransposedFlow<>(false, nextTask);
            ...
            for (final DirectedGraphBackNode<Task> nextBackNode : backNode.getBackNodes()) {
                ...
                final IsTransposedFlow<Task> nextIsBackFlow = new IsTransposedFlow<>(true, nextBackTask);
                bfsQueue.addLast(nextIsBackFlow);
                prePath.put(nextIsBackFlow, isBackFlow);
            }
        }
    ...
    private void dfsPostOrderReverseRecursive(final HashMap<Task, Boolean> skipOrNull,
        ...
        for (final DirectedGraphNode<Task> node : startNode.getNodes()) {
            if (skipOrNull != null) {
                if (skipOrNull.containsKey(node.getData())) {
                    continue;
                }
